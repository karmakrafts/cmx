...
if ("${EFI_TARGET_ARCH}" STREQUAL "x86_64")
    ...
    set(EFI_OBJCOPY_TARGET "efi-app-x86_64")
...
elseif ("${EFI_TARGET_ARCH}" STREQUAL "x86")
    ...
    set(EFI_OBJCOPY_TARGET "efi-app-i386")
    if (NOT "${EFI_TARGET_ARCH}" STREQUAL "${CMX_CPU_ARCH}")
...
elseif ("${EFI_TARGET_ARCH}" STREQUAL "arm64")
    ...
    set(EFI_TARGET_64_BIT ON)
    set(EFI_OBJCOPY_TARGET "efi-app-aarch64")
    if (NOT "${EFI_TARGET_ARCH}" STREQUAL "${CMX_CPU_ARCH}")
...
elseif ("${EFI_TARGET_ARCH}" STREQUAL "arm")
    ...
    set(EFI_OBJCOPY_TARGET "efi-app-arm")
    if (NOT "${EFI_TARGET_ARCH}" STREQUAL "${CMX_CPU_ARCH}")
...
elseif ("${EFI_TARGET_ARCH}" STREQUAL "riscv64")
    ...
    set(EFI_TARGET_64_BIT ON)
    set(EFI_OBJCOPY_TARGET "efi-app-riscv64")
    if (NOT "${EFI_TARGET_ARCH}" STREQUAL "${CMX_CPU_ARCH}")
...
macro(cmx_include_efi target access)
    ...
    if ("${EFI_TARGET_ARCH}" MATCHES "x86(_64)?")
        target_compile_options(${target} ${access} -mno-red-zone)
    endif ()
...
macro(cmx_add_esp_image target)
    ...
    add_custom_command(
            TARGET ${target}
            COMMAND ${MCOPY}
            ARGS -i "${image_file}" "${CMX_ESP_BOOT_FILE}" "::EFI/BOOT/${EFI_BOOT_FILE_NAME}"
            WORKING_DIRECTORY ${CMAKE_CURRENT_BINARY_DIR}
            BYPRODUCTS ${image_file})
    set(visited_paths "")
